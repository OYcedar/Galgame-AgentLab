...
临时 exe、dll、脚本 dump 和一次性分析文件放在当前游戏工作目录。
...
5. 脚本命令、标签、变量声明、注释和调试参数保留在内部映射中。
...
- 中文脚本使用当前 exe 已验证支持的目标编码。
- 只本地化可见对白、名字、菜单文本、时间卡和选项。
- 引擎命令保持半角 ASCII。
- 命令行、标签、变量名和函数调用保持原始语法形态。
...
- 如果封包超过嵌入槽大小，先压缩注释和非显示冗余内容，保留可见文本和命令完整。

有旧补丁时，把它作为风格参考；新补丁基于当前版本本体重建。
...
4. 确认新封包能放入槽位。
5. 写入新封包，并把剩余字节清零。
6. 包装器元数据修改后，实机确认结果 exe 能进入前台并到达标题菜单。
...
字号增大时同步减少行宽，并进入正式流程测试脚本字体设置和启动器设置。
